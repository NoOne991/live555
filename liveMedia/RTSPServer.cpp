...
void *myThreadFunc(void* parm)
{
    RTSPServer* rtspServer = ( RTSPServer*)parm;
    if (rtspServer == NULL){
        return NULL;
    }
    while (rtspServer->myflag == 1)
    {
	    socklen_t len = sizeof(rtspServer->clientSockaddr);
	    rtspServer->fMyClientSocket = accept(rtspServer->fMyServerSocket, (struct sockaddr *)&rtspServer->clientSockaddr, &len);
	    if (rtspServer->fMyClientSocket == -1)  
	    {
	        printf("accept error\n");
	        ::closeSocket(rtspServer->fMyServerSocket);
	        continue ;
	    }  
           HashTable* hash = rtspServer->getHashTable();
           if (hash != NULL && !hash->IsEmpty())
           {
           	char buf[5] = "PLAY";
    		sendto(rtspServer->fMyClientSocket, (char*)buf, 5, 0, (struct sockaddr*)&rtspServer->clientSockaddr, sizeof rtspServer->clientSockaddr);
           }
    }
    return NULL;
}
...
int RTSPServer::createMyServerSocket(){
    fMyServerSocket = socket(AF_INET, SOCK_STREAM, 0);
    if (fMyServerSocket < 0)
    {
        return -1;
    }

    int err,sock_reuse=1; 
    err=setsockopt(fMyServerSocket,SOL_SOCKET,SO_REUSEADDR,
		(char *)&sock_reuse,sizeof(sock_reuse)); 
    if(err!=0){
        ::closeSocket(fMyServerSocket);
        return -1;
    } 
    ...
    addr.sin_port = htons(nMyServerPort);
    
    ignoreSigPipeOnSocket(fMyServerSocket);
    
    if (bind(fMyServerSocket, (struct sockaddr *)&addr, sizeof(addr)) == -1)
    {
        printf("bind error\n");
        ::closeSocket(fMyServerSocket);
        return -1;
    ...
    if (listen(fMyServerSocket, 5) == -1)  
    {  
        printf("listen error\n");
        ::closeSocket(fMyServerSocket);
        return -1;
    }  
    if(pthread_create(&myThreadId, NULL, myThreadFunc, this) != 0)
    {
        printf("Create myThreadFunc fail !\n");
    }
    return fMyServerSocket;
  }
...
RTSPServer::RTSPServer(UsageEnvironment& env,
  ...
  : Medium(env),
    ...
    fMyServerSocket(-1), fMyClientSocket(-1), nMyServerPort(6665), myflag(1){  //add by yujinhong
...
RTSPServer::~RTSPServer() {
  ...
  delete fPendingRegisterRequests;
   ...
   //yujinhong
   myflag = 0;
   (void)pthread_join(myThreadId, NULL);
   ::closeSocket(fMyServerSocket);
...
::handleCmd_TEARDOWN(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  //yujinhong 
  //if the client number of sessions is 0, close encoder
  if (fOurServer.fClientSessions->IsEmpty() && fOurServer.fMyServerSocket > 0 && fOurServer.fMyClientSocket > 0)
  {
    //printf("client num=0\n");
    char buf[9] = "TEARDOWN";
    sendto(fOurServer.fMyClientSocket, (char*)buf, 9, 0, (struct sockaddr*)&fOurServer.clientSockaddr, sizeof fOurServer.clientSockaddr);
  }
}
...
::handleCmd_PLAY(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  if (!fOurServer.fClientSessions->IsEmpty() && fOurServer.fMyServerSocket > 0 && fOurServer.fMyClientSocket > 0)
  {
    //printf("client num=0\n");
    char buf[5] = "PLAY";
    sendto(fOurServer.fMyClientSocket, (char*)buf, 5, 0, (struct sockaddr*)&fOurServer.clientSockaddr, sizeof fOurServer.clientSockaddr);
  }
}
